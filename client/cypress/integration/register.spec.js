...
describe("Unauthenticated User", () => {
  ...
  it("redirects to login when the form is filled in and submitted", () => {
    cy.visit("http://localhost:3000/account");
    cy.url().should("match", /login/);
    cy.get("div.btn-group")
      .contains(/register/i)
  ...
  it("displays error message if submitted with a password less than 5 characters", () => {
    ...
    cy.get("div.container").contains(/Register/i);

    cy.get("#error").contains(/password/i);
    cy.get("#error").contains(/too short/i);
  });
});
